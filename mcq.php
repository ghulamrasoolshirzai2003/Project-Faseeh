...
<body>
    ...
    async function loadQuestion() {
        ...
        try {
            ...
            const text = await res.text(); // Get raw text first to check for PHP errors
            try {
                const data = JSON.parse(text);
                if (data.error) { 
                    card.innerHTML = `<div style="padding:20px; color:#ff7675;"><h3>❌ Data Error</h3><p>${data.error}</p><a href="level_select.php" class="btn-secondary" style="margin-top:15px; display:inline-block;">Go Back</a></div>`; 
                    return; 
                }
                currentData = data;
                renderQuestion(data);
            } catch (jsonError) {
                // This happens if PHP outputs a "Database Error" instead of JSON
                card.innerHTML = `<div style="padding:20px; color:#ff7675; text-align:left;"><h3>⚠️ Server Error</h3><div style="background:rgba(0,0,0,0.2); padding:10px; border-radius:10px; font-family:monospace; font-size:0.8rem; margin-top:10px; overflow-x:auto;">${text}</div><a href="level_select.php" class="btn-secondary" style="margin-top:15px; display:inline-block;">Go Back</a></div>`;
            }
        } catch (e) { 
            console.error(e); 
            card.innerHTML = `<p style="color:var(--danger);">Connection Error: ${e.message}</p>`;
        }
    }
    ...
    function showResults() {
        ...
        card.innerHTML = `
            <div class="round-complete">
                ...
                <a href="level_select.php" class="btn-secondary">🎮 Play Menu</a>
